...
export const currentDictIdAtom = atomWithStorage('currentDict', 'lektion01')
export const currentDictInfoAtom = atom<Dictionary>((get) => {
  const id = get(currentDictIdAtom)
  let dict = idDictionaryMap[id]
  // 如果 dict 不存在，则返回 lektion01. Typing 中会检查 DictId 是否存在，如果不存在则会重置为 lektion01
  if (!dict) {
    dict = idDictionaryMap.lektion01
  }
  return dict
})
